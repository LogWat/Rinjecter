...
pub unsafe extern "stdcall" fn overwrite(process: &Process) -> Result<(), &'static str> {
    ...
    let rb1: [u32; 20] = [
        ...
        0xEA0805C7, 0x4BEA08A3, 0xEA081589, 0xEA081539
        ];
    let rb2: [u32; 3] = [0x9000, 0x4B, 0x1004B];
    ...
    Process::write(process, 0x41DD9E, 0x3 as u32).unwrap();
    ...
    Process::write(process, 0x41F8E1, rb2[2]).unwrap();
    Process::write(process, 0x41F8E5, 0x0 as u32).unwrap();
    ...
    Process::write(process, 0x41FFF0, 0x3 as u32).unwrap();
